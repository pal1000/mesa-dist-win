...
@%msysloc%\usr\bin\pacman -Syu --noconfirm
@echo.
@%msysloc%\usr\bin\pacman -S python2-mako scons mingw-w64-%mingwabi%-clang flex bison patch --needed --noconfirm
@echo.
...
@IF NOT EXIST mesapatched.ini %msysloc%\usr\bin\patch -Nbp1 -i "%mesa%/mesa-dist-win/patches/s3tc.patch"
@IF NOT EXIST mesapatched.ini echo.
@IF NOT EXIST mesapatched.ini %msysloc%\usr\bin\patch -Nbp1 -i "%mesa%/mesa-dist-win/patches/link-ole32.patch"
@IF NOT EXIST mesapatched.ini echo.
...
@echo %buildcmd%
@echo.
@%msysloc%\usr\bin\bash --login %mesa%/mesa-dist-win/debug/build.sh
