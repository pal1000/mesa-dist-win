...
@IF "%shell%"=="4" %runmsys% $(/usr/bin/cygpath -m ${devroot})/${projectname}/buildscript/modules/clang32.sh
...
@IF /I "%msyscmd%"=="setup" IF %shell% EQU 1 %runmsys% pacman -S flex bison patch tar mingw-w64-i686-{python-mako,meson,pkgconf,vulkan-devel,libelf,zstd,gdb,llvm,cc} mingw-w64-x86_64-{python-mako,meson,pkgconf,vulkan-devel,libelf,zstd,gdb,llvm,cc} --needed
@IF /I "%msyscmd%"=="setup" IF %shell% GTR 1 %runmsys% pacman -S flex bison patch tar ${MINGW_PACKAGE_PREFIX}-{python-mako,meson,pkgconf,vulkan-devel,libelf,zstd,gdb,llvm,cc} --needed
