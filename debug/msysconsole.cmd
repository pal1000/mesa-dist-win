...
@IF /I "%msyscmd%"=="clearcache" for /f "tokens=*" %%a in ('dir /B "%msysloc%\var\cache\pacman\pkg"') do @for /f "tokens=1 delims=-" %%b in ("%%a") DO @IF NOT "%%b"=="bison" IF NOT "%%b"=="flex" IF NOT "%%b"=="m4" IF NOT "%%b"=="mingw" IF NOT "%%b"=="patch" IF NOT "%%b"=="tar" del %msysloc%\var\cache\pacman\pkg\%%a
@IF /I "%msyscmd%"=="cleancache" %msysloc%\usr\bin\bash --login -c "pacman -Sc --noconfirm"
@IF /I "%msyscmd%"=="cleancache" for /f "tokens=*" %%a in ('dir /B "%msysloc%\var\cache\pacman\pkg"') do @for /f "tokens=1 delims=-" %%b in ("%%a") DO @IF NOT "%%b"=="bison" IF NOT "%%b"=="flex" IF NOT "%%b"=="m4" IF NOT "%%b"=="mingw" IF NOT "%%b"=="patch" IF NOT "%%b"=="tar" del %msysloc%\var\cache\pacman\pkg\%%a
