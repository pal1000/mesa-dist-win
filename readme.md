...
Since v17.0.1.391 in-place upgrade is fully supported. Since v17.0.1.391-2 S3 texture compression is supported. v17.0.4.391-1 requires uninstall of previous versions. Applications requiring OpenGL 3.2 or newer may need [Manual OpenGL context configuration](#manual-opengl-context-configuration).

# Manual OpenGL context configuration
With release of [OpenGL 3.1](https://en.wikipedia.org/wiki/OpenGL#OpenGL_3.1) features marked as deprecated in OpenGL 3.0 have been removed. Most proprietary drivers opted to implement the exemption from this rule offered in the form of GL_ARB_compatibility extension. Due to complexity and especially lack of correct implementation tests, Mesa3D opted for an easier solution in short and medium term. Mesa3D implemented core profile and core+forward compatible contexts introduced with OpenGL 3.2 as an immediate alternative to GL_ARB_compatibility to support OpenGL 3.0 and up and use the already existing compatibility context to implement GL_ARB_compatibility functionality slowly but steadily. Since Mesa 18.1, OpenGL 3.1 is available to compatibility context via GL_ARB_compatibility. Because of GL_ARB_compatibility limits in Mesa3D, OpenGL compatibility context won't get above OpenGL 3.0 for Mesa 18.0 and 3.1 for Mesa 18.1.

Context types description:

Context type | Description
------------ | -----------
Compatibility context | This is the original OpenGL context. Intended for old applications requiring up to and including OpenGL 3.1 or which use both old and recent OpenGL features. It can be used for OpenGL 3.2 and up to simulate GL_ARB_compatibility, but being unsupported for OpenGL 3.2 and up it is neither exposed as GL_ARB_compatibility nor it can be automatically requested by applications via context flags for OpenGL 3.2 and up.
...
Default values: 140 if MESA_GL_VERSION_OVERRIDE is undefined or 330 otherwise.

# How to set environment variables
Under Windows the easiest way to set environment variables is by writing batch files for every application requiring OpenGL >= 3.2 that is using Mesa or if your want to use swr instead of llvmpipe for Desktop OpenGL.
...
There no conflict between GALLIUM_DRIVER variable used to change the desktop OpenGL driver and manual OpenGL context configuration. You can mix them if necessary.
