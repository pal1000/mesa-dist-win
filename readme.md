...
# Downloads
Mesa 22.0.1 builds with Visual Studio and MSYS2 Mingw-w64 are now available in [releases section](https://github.com/pal1000/mesa-dist-win/releases).
# Known issues
...
- Vulkan driver for AMD graphics (radv) is available in both MSVC and MinGW packages since 21.2.0. 32-bit binary of it is available since Mesa 22.0. File names: `radeon_icd.x86_64.json`, `radeon_icd.x86.json` and `vulkan_radeon.dll`. Deployment for Vulkan drivers is done through Vulkan runtime using ICD discovery method. Note that Vulkan runtime commes bundled with graphics drivers supporting Vulkan so manually installing it may not be necessary.
