...
# Downloads
Mesa 21.2.0 builds with Visual Studio and MSYS2 Mingw-w64 are now available in [releases section](https://github.com/pal1000/mesa-dist-win/releases).
# Known issues
...
- Vulkan driver for AMD graphics (radv) is available in both MSVC and MinGW packages since 21.2.0, but only for [x64](https://gitlab.freedesktop.org/mesa/mesa/-/issues/5081). File names: radeon_icd.x86_64.json, vulkan_radeon.dll (MSVC package), libvulkan_radeon.dll (MinGW package). Deployment for Vulkan drivers is done through Vulkan runtime using ICD discovery method. Note that Vulkan runtime commes bundled with graphics drivers supporting Vulkan so manually installing it may not be necessary.
- D3D10 software renderer is available in MSVC package since 21.2.0. File name: d3d10sw.dll. This is a drop in replacement for Microsoft WARP and unfortunateely there is no clean way of [deploying](https://gitlab.freedesktop.org/mesa/mesa/-/blob/main/src/gallium/targets/d3d10sw/README.md) it.
