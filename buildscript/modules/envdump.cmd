...
@FOR /F "tokens=2 delims= " %%a IN ('echo %winver%') DO @set winver=%%a
@FOR /F "tokens=1-3 delims=." %%a IN ('echo %winver%') DO @set winver=%%a.%%b.%%c
...
@rem Get Git version
@IF NOT %gitstate%==0 FOR /F "tokens=3" %%a IN ('git --version') do @set gitver=%%a
...
@IF %toolchain%==msvc echo --------------->>%devroot%\%projectname%\buildinfo\msvc.txt
@IF %toolchain%==msvc FOR /F "skip=2 tokens=* delims=" %%a IN ('%pythonloc% -W ignore -m pip list --disable-pip-version-check') do @echo %%a>>%devroot%\%projectname%\buildinfo\msvc.txt
@IF %toolchain%==msvc echo.>>%devroot%\%projectname%\buildinfo\msvc.txt
...
@IF %toolchain%==msvc IF NOT "%cmakestate%"=="0" IF NOT "%cmakestate%"=="" for /f "tokens=3" %%a IN ('cmake --version') do @if defined exitloop (
set "exitloop="
...
@IF %toolchain%==msvc IF NOT "%ninjastate%"=="0" IF NOT "%ninjastate%"=="" for /f %%a IN ('ninja --version') do @echo Ninja %%a>>%devroot%\%projectname%\buildinfo\msvc.txt

@rem Get LLVM version
@IF %toolchain%==msvc IF EXIST %devroot%\llvm\build\%hostabi%\bin\llvm-config.exe FOR /F %%a IN ('%devroot%\llvm\build\%hostabi%\bin\llvm-config.exe --version') do @echo LLVM %%a>>%devroot%\%projectname%\buildinfo\msvc.txt
...
@IF %toolchain%==msvc IF NOT "%flexstate%"=="0" IF NOT "%flexstate%"=="" for /f "tokens=2" %%a IN ('win_flex --version') do @echo flex %%a>>%devroot%\%projectname%\buildinfo\msvc.txt
@IF %toolchain%==msvc IF NOT "%flexstate%"=="0" IF NOT "%flexstate%"=="" set exitloop=1
@IF %toolchain%==msvc IF NOT "%flexstate%"=="0" IF NOT "%flexstate%"=="" for /f "tokens=4" %%a IN ('win_bison --version') do @if defined exitloop (
set "exitloop="
...
@IF %toolchain%==msvc IF %pkgconfigstate% GTR 0 FOR /F %%a IN ('%pkgconfigloc%\pkg-config.exe --version') do @set pkgconfigver=%%a
