...
@rem Get libclc version
@IF EXIST "%llvminstloc%\clc\share\pkgconfig\mesa-libclc.pc" IF %toolchain%==msvc echo Mesa-libclc %mesaliibclcver%>>"%devroot%\%projectname%\buildinfo\msvc.txt"
@IF EXIST "%llvminstloc%\clc\share\pkgconfig\mesa-libclc.pc" IF NOT %toolchain%==msvc echo Mesa-libclc %mesaliibclcver%>>"%devroot%\%projectname%\buildinfo\mingw.txt"
