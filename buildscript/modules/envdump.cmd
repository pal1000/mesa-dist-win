...
@set vksdkver=0
@for /f delims^=^ eol^= %%a in ('dir /A:D /B "%SystemDrive%\VulkanSDK\"') do @IF EXIST "%SystemDrive%\VulkanSDK\%%a\" set /a vksdkver+=1
@IF %vksdkver% GTR 1 set vksdkver=0
@IF %vksdkver% EQU 1 for /f delims^=^ eol^= %%a in ('dir /A:D /B "%SystemDrive%\VulkanSDK\"') do @IF EXIST "%SystemDrive%\VulkanSDK\%%a\" set vksdkver=%%a
