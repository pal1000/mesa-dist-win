...
@%msysloc%\%MSYSTEM%\bin\strip --only-keep-debug %devroot:\=/%/%projectname%/debugsymbols/%abi%/osmesa-gallium/osmesa.dll.symbols
@%msysloc%\%MSYSTEM%\bin\strip --strip-debug --strip-unneeded %devroot:\=/%/%projectname%/bin/%abi%/osmesa-gallium/osmesa.dll
)
...
@%msysloc%\%MSYSTEM%\bin\strip --only-keep-debug %devroot:\=/%/%projectname%/debugsymbols/%abi%/osmesa-swrast/osmesa.dll.symbols
@%msysloc%\%MSYSTEM%\bin\strip --strip-debug --strip-unneeded %devroot:\=/%/%projectname%/bin/%abi%/osmesa-swrast/osmesa.dll
)
@forfiles /p %devroot%\%projectname%\bin\%abi% /m *.dll /c "cmd /c copy @path %devroot%\%projectname%\debugsymbols\%abi%\@file.symbols"
@forfiles /p %devroot%\%projectname%\bin\%abi% /m *.dll /c "cmd /c %msysloc%\%MSYSTEM%\bin\strip --only-keep-debug %devroot:\=/%/%projectname%/debugsymbols/%abi%/@file.symbols"
@forfiles /p %devroot%\%projectname%\bin\%abi% /m *.dll /c "cmd /c %msysloc%\%MSYSTEM%\bin\strip --strip-debug --strip-unneeded %devroot:\=/%/%projectname%/bin/%abi%/@file"
@forfiles /p %devroot%\%projectname%\bin\%abi% /m *.exe /c "cmd /c %msysloc%\%MSYSTEM%\bin\strip --strip-debug --strip-unneeded %devroot:\=/%/%projectname%/bin/%abi%/@file"
@echo.
@echo Done.
@echo.