...
@IF NOT %toolchain%==msvc IF EXIST "%msysloc%\%LMSYSTEM%\bin\strip.exe" for /f delims^=^ eol^= %%a in ('dir /b "%devroot%\%projectname%\bin\%abi%\*.dll" 2^>^&1') do @IF EXIST "%devroot%\%projectname%\bin\%abi%\%%~nxa" IF /I NOT "%%~na"=="dxil" IF /I NOT "%%~na"=="openclon12" IF /I NOT "%%~na"=="WinPixEventRuntime" (
