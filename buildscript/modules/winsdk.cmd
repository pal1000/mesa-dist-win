...
@IF NOT "%winsdk%"=="null" IF "%1"=="dxil" for /f "tokens=* delims=" %%a in ('dir /b /s "%winsdk%\bin"') do @if "%%~nxa"=="dxil.dll" (
@set dxilloc=%%~dpa
...
@IF NOT "%winsdk%"=="null" IF "%1"=="wdk" IF EXIST %devroot%\mesa\include\winddk\.gitignore for /f "tokens=* delims=" %%a in ('type %devroot%\mesa\include\winddk\.gitignore') do @(
set /a winwdkcount-=1
for /f "tokens=* delims=" %%b in ('dir /b /s "%winsdk%\%%a" 2^>nul') do @if /I "%%~nxa"=="%%~nxb" set /a winwdkcount+=1
)
