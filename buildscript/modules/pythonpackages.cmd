...
@IF NOT %pypack%==0 IF NOT %pypack%==meson %pythonloc% -m pip install -U %pypack%
@IF %pypack%==meson %pythonloc% -m pip install -U %pypack%==0.47.2
...
@if /I "%pyupd%"=="y" for /F "skip=2 delims= " %%m in ('%pythonloc% -m pip list -o --disable-pip-version-check') do @IF NOT "%%m"=="meson" %pythonloc% -m pip install -U "%%m"
