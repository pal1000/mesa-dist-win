...
@IF %toolchain%==gcc FOR /F "tokens=* USEBACKQ" %%a IN (`%msysloc%\usr\bin\bash --login -c "%LLVM%/bin/llvm-config --link-static --libnames engine coroutines"`) DO @SET llvmlibs=%%~a
