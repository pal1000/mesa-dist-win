...
@IF %toolchain%==gcc FOR /F "tokens=* USEBACKQ" %%a IN (`%msysloc%\usr\bin\bash --login -c "${MINGW_PREFIX}/bin/llvm-config --version" 2^>^&1`) DO @SET llvmver=%%~a
...
IF %toolchain%==gcc echo llvmloc = run_command^('%devroot:\=/%/%projectname%/buildscript/modules/msysllvmloc.cmd'^).stdout^(^).strip^(^)
