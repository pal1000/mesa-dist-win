...
@echo _search = join_paths('%LLVM:\=/%', 'lib') >> %mesa%\mesa\subprojects\llvm\meson.build
...
@echo   include_directories : include_directories(join_paths('%LLVM:\=/%','include')), >> %mesa%\mesa\subprojects\llvm\meson.build
...
@echo ) >> %mesa%\mesa\subprojects\llvm\meson.build
@echo. >> %mesa%\mesa\subprojects\llvm\meson.build
@echo irbuilder_h = files(join_paths('%LLVM:\=/%','include/llvm/IR/IRBuilder.h')) >> %mesa%\mesa\subprojects\llvm\meson.build