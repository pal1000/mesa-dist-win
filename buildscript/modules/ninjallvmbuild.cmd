...
@set llvmlibstotal=0
@FOR /F "tokens=4 USEBACKQ" %%a IN (`%devroot%\llvm\%abi%\bin\llvm-config.exe --link-static --libnames 2^>^&1`) DO @set /a llvmlibstotal+=1
@set llvmlibscount=0
@FOR /F "tokens=4 USEBACKQ" %%a IN (`%devroot%\llvm\%abi%\bin\llvm-config.exe --link-static --libnames 2^>^&1`) DO @(
