...
@IF /I NOT "%useninja%"=="y" IF /I %PROCESSOR_ARCHITECTURE%==AMD64 echo Performing VA-API build with : msbuild /p^:Configuration=release,Platform=x64 INSTALL.vcxproj /m^:%throttle%
@IF /I NOT "%useninja%"=="y" IF /I %PROCESSOR_ARCHITECTURE%==x86 echo Performing VA-API build with : msbuild /p^:Configuration=release,Platform=Win32 INSTALL.vcxproj /m^:%throttle%
@IF /I NOT "%useninja%"=="y" echo.
@IF /I NOT "%useninja%"=="y" IF /I %PROCESSOR_ARCHITECTURE%==AMD64 msbuild /p^:Configuration=release,Platform=x64 INSTALL.vcxproj /m^:%throttle%
@IF /I NOT "%useninja%"=="y" IF /I %PROCESSOR_ARCHITECTURE%==x86 msbuild /p^:Configuration=release,Platform=Win32 INSTALL.vcxproj /m^:%throttle%
