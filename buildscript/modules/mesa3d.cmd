...
@if %botmode% LEQ 0 set branch= -b main
...
@if %botmode% LEQ 0 set usezstd=n
...
@if %botmode% LEQ 0 set linkmingwdynamic=n
...
@if %botmode% LEQ 0 set llvmless=n
...
@if %botmode% LEQ 0 set glswrast=n
...
@if %botmode% LEQ 0 set lavapipe=n
@set canlavapipe=1
...
@IF %glslangval% EQU 0 IF %intmesaver% GEQ 22200 set canradv=0
...
@if %botmode% LEQ 0 set spirvtodxil=n
...
@if %PKG_CONFIG_ST% EQU 1 IF %intmesaver% GEQ 25300 set buildconf=%buildconf% -Dspirv-tools=%mesonbooltrue%
@if %PKG_CONFIG_ST% EQU 0 IF %intmesaver% GEQ 25300 set buildconf=%buildconf% -Dspirv-tools=%mesonboolfalse%
...
@if %botmode% LEQ 0 set mesatests=n
@set canmesatests=1
...
@IF %canmesatests% EQU 1 IF %intmesaver% GEQ 24153 IF %intmesaver% LSS 25200 call "%devroot%\%projectname%\bin\modules\prompt.cmd" mesatests "Do you want to build unit tests and dynamic pipe loader (y/n):"
...
@IF %canmesatests% EQU 1 IF %intmesaver% GEQ 25200 call "%devroot%\%projectname%\bin\modules\prompt.cmd" mesatests "Do you want to build unit tests (y/n):"
...
@if /I "%radv%"=="y" if /I "%mesatests%"=="y" IF %glslangval% EQU 1 set buildconf=%buildconf:~0,-5%true
...
@if /I "%mesatests%"=="y" set buildconf=%buildconf%dlclose-skip,glsl,nir,
...
@IF %toolchain%==msvc call %vsenv% %WINSDK_VER% %vsabi% -vcvars_ver=%msvcpp%
