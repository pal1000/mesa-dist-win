...
@IF NOT %toolchain%==msvc if /I NOT "%llvmless%"=="y" FOR /F "tokens=* delims=" %%a IN ('%msysloc%\usr\bin\bash --login -c "${MINGW_PREFIX}/bin/llvm-config --has-rtti" 2^>^&1') DO @IF /I NOT "%%a"=="YES" IF /I NOT "%%a"=="NO" set llvmconfigbusted=1
...
@FOR /F "tokens=2 delims= " %%a IN ('%msysloc%\usr\bin\bash --login -c "/usr/bin/pacman -Q ${MINGW_PACKAGE_PREFIX}-llvm"') DO @FOR /F "tokens=1 delims=-" %%b IN ("%%~a") DO @SET llvmver=%%b
...
@FOR /F "tokens=2 delims= " %%a IN ('%msysloc%\usr\bin\bash --login -c "/usr/bin/pacman -Q ${MINGW_PACKAGE_PREFIX}-zlib"') DO @FOR /F "tokens=1 delims=-" %%b IN ("%%~a") DO @SET zlibver=%%b
...
@FOR /F "tokens=2 delims= " %%a IN ('%msysloc%\usr\bin\bash --login -c "/usr/bin/pacman -Q ${MINGW_PACKAGE_PREFIX}-zstd"') DO @FOR /F "tokens=1 delims=-" %%b IN ("%%~a") DO @SET zstdver=%%b
