...
@IF %mesabldsys%==meson IF %toolchain%==gcc IF %abi%==x86 set mesonloc=%msysloc%\usr\bin\bash --login -c "cd ${devroot}/mesa;/mingw32/bin/meson
@IF %mesabldsys%==meson IF %toolchain%==gcc IF %abi%==x64 set mesonloc=%msysloc%\usr\bin\bash --login -c "cd ${devroot}/mesa;/mingw64/bin/meson
